...
int wordcount __attribute__ ((aligned (4096)));
keyvalue input_data[SIZE_DB] __attribute__ ((aligned (256*4096)));
keyvalue mapped_data[SIZE_DB] __attribute__ ((aligned (256*4096)));
keyvalue reduced_data[SIZE_DB] __attribute__ ((aligned (256*4096)));
keyvalue d[SIZE_DB] __attribute__ ((aligned (256*4096)));


//keyvalue* input_data;
//keyvalue* mapped_data;
//keyvalue* reduced_data;
...
void psu_mr_setup(unsigned int nthreads) {
	//input_data = psu_dsm_malloc("input_data", wordcount*sizeof(keyvalue));
	//mapped_data = psu_dsm_malloc("mapped_data", wordcount*sizeof(keyvalue));
	//reduced_data = psu_dsm_malloc("reduced_data", wordcount*sizeof(keyvalue));
	init_globals_mr();
...
void setup_input_data() {
	...
	while (file >> token) {
		...
		//i = i % SIZE_DB;
		if (i==SIZE_DB){
			cout << "Database size not large enough!" << endl;
			break;
		}
		j++;
		if (j%100 == 0){
			cout << "read line: " << j << endl;
		}
	}
	wordcount = i;
}
...
int main(int argc, char* argv[])
{
	...
	psu_init_lock(0);
	psu_init_lock(1);
	psu_dsm_register_datasegment(&a, 4096*(12+(256*4)));   

...
void * kmeans_mapper(void * ptr) {
	...
	int start_keyvalue_idx = num_clusters + machine_id * (num_data_points/NUM_THREADS);
	int end_keyvalue_idx = num_clusters + (machine_id + 1) * (num_data_points/NUM_THREADS);
...
void mr_master() {
	...
	*/
	...
	if (NUM_THREADS != num_clusters) {
		cout << "not enough worker threads!" << endl;
		return;
	}
	
	psu_mr_map(kmeans_mapper, input_data, mapped_data);
	printf("Done mapping!\n");
	...
	*/
	...
	psu_mr_reduce(kmeans_reducer, mapped_data, reduced_data);
